...
To quantify generalization, we computed KID against the training distribution at four rollout lengths (250, 500, 1000, and 2000 frames), where 250 frames is the training clip length.

<Figure>
  <Picture slot="figure" src={kidRolloutLength} alt="KID across rollout lengths" />
  <Fragment slot="caption">
    **Generalization KID across rollout lengths.** Lower is better. Code2Act (light blue) outperforms Mimic-MJX (dark blue) at all four durations. Error bars = std across 3 VAE seeds.
  </Fragment>
</Figure>

Code2Act outperforms Mimic-MJX at every rollout length, and the gap widens monotonically with duration. At the training clip length (250 frames), both methods are closest in performance, but as rollouts extend beyond the training horizon, Mimic-MJX accumulates drift at clip boundaries &mdash; it must track novel reference trajectories frame-by-frame and has no mechanism for long-range coherence. Code2Act's discrete syllable interface provides a natural abstraction: as long as new behavior decomposes into the same syllable vocabulary, each code maps to its learned motor program regardless of duration. The syllable bottleneck, rather than limiting expressiveness, acts as a regularizer that enables robust transfer to new behavioral contexts.
