...
KPMS segments behavior into fine-grained syllables because it can only decompose what is present in the data &mdash; brief, overlapping motion fragments. The syllable trajectory grid below illustrates this: each syllable captures a short, stereotyped motion primitive (a stride fragment, a head turn, a postural shift) rather than a complete behavior.

<Figure>
  <Picture slot="figure" src={syllableGrid} alt="KPMS syllable trajectory grid" />
  <Fragment slot="caption">
    **KPMS Syllable Trajectories.** Top-down (XY) and side (XZ) projections of keypoint trajectories for each syllable, showing the fine-grained motion primitives that KPMS extracts from continuous behavior. Each syllable captures a brief, stereotyped motif.
  </Fragment>
</Figure>

With a generative controller like Code2Act, we can hold a single code constant and observe its effect in isolation (see [Within-Syllable Hidden Dynamics](#within-syllable-hidden-dynamics) for the corresponding hidden state video). This reveals that each code drives only a single type of motion &mdash; a repetitive gait cycle, a static hold, or a brief postural shift &mdash; rather than the diverse repertoire that real animals produce. Coherent behavioral sequences emerge only when codes compose over time, with transitions between codes creating the richness of naturalistic movement.
...
<TwoColumns>
  <Figure slot="left">
    ...
    <Fragment slot="caption">
      **Walk pose instantiation.** Violin plots of XY speed, root Z height, and forelimb/hindlimb angular velocity (shoulder/elbow/wrist vs hip/knee/ankle) per behavior. Individual dots = per-clip values.
    </Fragment>
  </Figure>
  <Figure slot="right">
    <Picture slot="figure" src={kinematicSigHigh} alt="Kinematic signatures from rear pose" />
    <Fragment slot="caption">
      **Rear pose instantiation.** Same metrics from a rear-like starting pose. The pattern holds: walk codes produce the highest locomotion and joint activity, immobility codes remain still, and rear codes drive vertical posture with strong hindlimb engagement.
    </Fragment>
  </Figure>
</TwoColumns>
...
Code sequences are instructional at both coarse and fine scales: specified sequences produce expected behavior profiles (e.g., greater root displacement for walking codes) as well as Mimic-MJX's dominant gait frequency across hind limb joints in walking clips. The power spectral density of hind limb joint angles shows that Code2Act captures the dominant ~2 Hz gait frequency bilaterally, with proximal joints (hip) showing tight spectral overlap and distal joints (knee) showing expected divergence.
