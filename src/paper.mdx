...
To quantify generalization, we computed KID against the training distribution at three rollout lengths (500, 1000, and 2000 frames). Code2Act consistently produces behavior closer to real than Mimic-MJX across all durations.

<Figure>
  <Picture slot="figure" src={kidRolloutLength} alt="KID across rollout lengths" />
  <Fragment slot="caption">
    **Generalization KID across rollout lengths.** Lower is better. Code2Act (light blue) outperforms Mimic-MJX (dark blue) at all three durations. Error bars = std across 3 VAE seeds.
  </Fragment>
</Figure>

The gap between Code2Act and Mimic-MJX widens at longer rollouts. This is expected: Mimic-MJX was trained on 500-frame clips, so at the training length (500 frames) both methods perform similarly. As rollouts extend beyond the training horizon, Mimic-MJX must track novel reference trajectories frame-by-frame and accumulates drift at clip boundaries, while Code2Act's discrete syllable interface provides a natural abstraction &mdash; as long as new behavior decomposes into the same syllable vocabulary, each code maps to its learned motor program regardless of duration. The syllable bottleneck, rather than limiting expressiveness, acts as a regularizer that enables robust transfer to new behavioral contexts.
