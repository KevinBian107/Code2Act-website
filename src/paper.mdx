...
Each individual syllable code drives only a single type of motion &mdash; a repetitive gait cycle, a static hold, or a brief postural shift &mdash; rather than the diverse, context-dependent behavior that real animals produce. Coherent behavioral sequences emerge only when codes compose over time, with transitions between codes creating the richness of naturalistic movement.
